...
OUTPUT_LOC="/rds/user/cs-burc1/hpc-work/datasets/turkish/backtranslation/sampling"
OUTPUT="sampling-bt-$TODAY.en"
...
$MARIAN_DECODER -m $MODEL1 $MODEL2 $MODEL3 $MODEL4  --devices $GPUS -v $VOCAB $VOCAB --beam-size 1 --input $INPUT --output $OUTPUT_LOC/$OUTPUT --max-length 150 --max-length-crop --mini-batch 8 --maxi-batch 100 --maxi-batch-sort src --workspace 15000 --quiet-translation --output-sampling
