...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  1 16:05:15 2021
...
class TreeLSTMCell(nn.Module):
    ...
    def reduce_func(self, nodes):
        ...
        # equation (2)
        u_result = torch.matmul(
            h_cat, self.U_f.weight[:last_dim, :last_dim]) + self.U_f.bias[:last_dim].unsqueeze(0)
        f = torch.sigmoid(u_result).view(*nodes.mailbox['h'].size())
        # second term of equation (5)
        c = torch.sum(f * nodes.mailbox['c'], 1)
        iou_result = torch.matmul(
            h_cat, self.U_iou.weight[:, :last_dim].transpose(0, 1))
        if OPTS.w_childnorm:
...
class DecoderTreeLSTMCell(TreeLSTMCell):
    ...
    def reduce_func(self, nodes):
        ...
        if "extra_input_depth_2" in nodes.data and int(
                (depth_mask == 2).any().numpy()) > 0:
        ...
        if "extra_input_depth_1" in nodes.data and int(
                (depth_mask == 1).any().numpy()) > 0:
        ...
        for pos in range(pos_data.max() + 1):
            ...
            pos_result = torch.matmul(
                pos_h_cat, self.U_f.weight[pos * self.h_size: (pos+1) * self.h_size])
            pos_result += self.U_f.bias[pos * self.h_size: (pos+1) * self.h_size].unsqueeze(0)
            u_result[pos_mask] = pos_result
            pos_iou_result = torch.matmul(
                pos_h_cat, self.U_iou.weight[:, pos * self.h_size: (pos+1) * self.h_size].transpose(0, 1))
